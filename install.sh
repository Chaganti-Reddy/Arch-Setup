...
# paru -S betterlockscreen-git brave-bin floorp-bin ccrypt didyoumean-git github-desktop-bin networkmanager-dmenu-git visual-studio-code-bin preload --noconfirm
...
cd ./Extras/Extras/waldl-master/ && sudo make install && cd ~/dotfiles
...
sudo cp ./Extras/Extras/etc/bash.bashrc /etc/
sudo cp ./Extras/Extras/etc/DIR_COLORS /etc/
sudo cp ./Extras/Extras/etc/mpd.conf /etc/
sudo cp ./Extras/Extras/etc/nanorc /etc/
