  ...
  where
    ...
    manageTerm = customFloating $ W.RationalRect l t w h
               where
                 ...
                 t = 0.95 -h
                 l = 0.95 -w
    spawnMocp  = myTerminal ++ " -t mocp -e ncmpcpp"
    findMocp   = title =? "mocp"
    manageMocp = customFloating $ W.RationalRect l t w h
               where
                 ...
                 t = 0.95 -h
                 l = 0.95 -w
    spawnHtop  = myTerminal ++ " -t mocp -e htop"
    findHtop   = title =? "htop"
...
myKeys c =
  ...
  , ("M-C-,", addName "Switch focus prev tab" $  onGroup W.focusDown')]
    ...
    -- KB_GROUP Useful programs to have a keybinding for launch
        ...
        , ("M-p", addName "Open PowerMenu"  $ spawn "rofi -show p -modi p:~/.local/bin/rofi-power-menu")
    ...
    -- KB_GROUP Grid Select (CTR-g followed by a key)
        ^++^ subKeys "GridSelect"
        [ ("C-g g", addName "My App Grid" $ spawnSelected' myAppGrid)                 -- grid select favorite apps
        , ("C-g t", addName "goto Selected Window" $ goToSelected $ mygridConfig myColorizer)  -- goto selected window
        , ("C-g b", addName "Bring Selected Window" $ bringSelected $ mygridConfig myColorizer) -- bring selected window
        , ("C-g p", addName "Grid Select Personal Apps" $ spawnSelected' myPersonalGrid)                 -- grid select personal apps
        ]
    ...
    -- Toggle them to hide and it sends them back to hidden workspace (NSP).
        ...
        , ("M-s m", addName "Toggle scratchpad Mpc" $ namedScratchpadAction myScratchPads "mocp")
    ...
    -- KB_GROUP Controls for mpc music player (SUPER-u followed by a key)
        ^++^ subKeys "Mpc music player"
        [ ("M-u p", addName "Mpc Music Player" $ spawn "mpc toggle")
        , ("M-u l", addName "Mpc Play Next" $ spawn "mpc next")
        , ("M-u h", addName "Mpc Play Prev" $ spawn "mpc prev")
        , ("M-u <Space>", addName "Mpc Toggle" $ spawn "mpc toggle")]
  ...
  [ ("<XF86AudioPlay>", addName "mpc play"           $ spawn "mpc toggle")
  ...
  , ("<XF86AudioLowerVolume>", addName "Lower vol"    $ spawn "pamixer -d 3")
  , ("<XF86AudioRaiseVolume>", addName "Raise vol"    $ spawn "pamixer -i 3")
